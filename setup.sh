...
docker build -t res/apache_rp ./docker-images/apache-reverse-proxy-image/
...
docker run -d res/express-js
docker run -d res/express-js
docker run -d res/express-js
docker run -d --name express-dynamic res/express-js

echo "##### RUN REVERSE PROXY CONTAINER #################"
static_ip=`docker inspect --format '{{ .NetworkSettings.IPAddress }}' apache-static`
dynamic_ip=`docker inspect --format '{{ .NetworkSettings.IPAddress }}' express-dynamic`
